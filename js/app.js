...
function renderColaboradores(){
  ...
  document.querySelector('#tblColaboradores tbody').innerHTML = Store.data.colaboradores.map(c=>{
    ...
    const anoSaida = c.saidaAnoId ? Store.getAno(c.saidaAnoId) : null;
    const saidaTxt = anoSaida
      ? `${formatarValorInicio(anoSaida, c.saidaMes)} <span class="badge inativo">Inativo</span>`
    ...
    <tr>
      ...
      <td class="mono small">${saidaTxt}</td>
...
function renderMembrosProjeto(projeto){
  ...
  // da empresa até este (ano, mês).
  const colaboradores = Store.data.colaboradores.filter(c => Store.colaboradorAtivoEm(c, anoDoProjeto.ano, ctx.mes));
...
function renderColaboradorDetalhe(){
  ...
  if(!Store.colaboradorAtivoEm(colab, anoObj.ano, ctx.mes)){
    tbody.innerHTML = '';
    emptyHint.hidden = false;
...
// Espelho de syncProjMesFim: o campo "Data de saída" só aparece quando o
// colaborador NÃO está marcado como "Ativo".
...
el('formColaborador').addEventListener('submit', e=>{
  ...
  // Data de saída só é lida (e exigida) quando "Ativo" está desmarcado.
  let saidaAnoId = null, saidaMes = null;
  if(!el('colabAtivo').checked){
    const saidaTexto = el('colabSaida').value.trim();
    const saidaDigitada = parseValorInicio(saidaTexto);
    if(!saidaDigitada){ toast('Digite a data de saída no formato mm/aaaa (ex.: 11/2025), ou marque "Ativo".'); return; }
    let anoSaidaObj = Store.getAnoPorNumero(saidaDigitada.ano);
    if(!anoSaidaObj){
      const resultado = Store.criarAno(saidaDigitada.ano);
      anoSaidaObj = resultado.ok ? resultado.ano : Store.getAnoPorNumero(saidaDigitada.ano);
    }
    if(!anoSaidaObj){ toast('Não foi possível registrar esse ano.'); return; }
    saidaAnoId = anoSaidaObj.id;
...
document.querySelector('#page-colaboradores').addEventListener('click', e=>{
  ...
  if(action==='editar-colab'){
    ...
    el('colabAtivo').checked = !anoSaidaAtual;
